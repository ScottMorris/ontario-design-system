...
```scss
.ontario-button { }                  // Block
.ontario-button__icon { }            // Element  
.ontario-button--primary { }         // Modifier
.ontario-button__icon--large { }     // Element modifier
```
...
test('should emit event on click', async ({ page }) => {
  ...
  const eventFired = await page.evaluate(() => {
    return new Promise(resolve => {
      document.querySelector('ontario-button')
        .addEventListener('buttonClick', () => resolve(true));
